    ...
    const columns = [
        {
            ...
            dataIndex: 'asset',
            key: 'asset',
            render: (text) => (<div className='font-bold text-[16px] flex items-center' >
                <Image src={findIcon(text[0])} style={{ width: 'auto', maxHeight: '4rem' }}></Image>
                <div className='ml-[10px]'>
                    <div>{text[1]}</div><div className='font-normal text-[#c8cad3] text-[16px] items-center'>{text[0]}</div>
                </div>
            </div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[12px] relative bottom-[-10px]  text-center' text-center>Total supplied</div>,
            dataIndex: 'total_supplied',
            key: 'total_supplied',
            render: (text) => (<div className='font-bold text-[16px]' ><div className='text-center'>{text[1]}</div><div className='font-normal text-[#c8cad3] text-[16px] text-center'>${text[0]}</div></div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[12px] relative bottom-[-10px] text-center'>Supply APY</div>,
            dataIndex: 'supply_apy',
            key: 'supply_apy',
            render: (text) => (<div className='font-bold text-[16px]'><div className='text-center'>{text}%</div></div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[12px] relative bottom-[-10px] text-center'>Total borrowed</div>,
            dataIndex: 'total_borrowed',
            key: 'total_borrowed',
            render: (text) => (<div className='font-bold text-[16px]'><div className='text-center'>{text[1]}</div><div className='font-normal text-[#c8cad3] text-[16px] text-center'>${text[0]}</div></div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[12px] relative bottom-[-10px] text-center'>Borrow APY,variable</div>,
            dataIndex: 'variable',
            key: 'variable',
            render: (text) => (<div className='font-bold text-[16px] text-center'>{text}%</div>)
        },
        {
            title: <div className=' text-[#c8cad3] text-[12px] relative bottom-[-10px] text-center'>Borrow APY,stable</div>,
            dataIndex: 'stable',
            key: 'stable',
            render: (text) => (<div className='font-bold text-[16px] text-center'>{text}%</div>)
        },
        {
            ...
            render: (text, record) => (<div className='flex font-semibold '>
                ...
                <Button className='text-[14px] py-[5px] px-[10px] rounded-[6px] border border-solid border-[#EAEBF0] cursor-pointer flex items-center justify-center' onClick={() => router.push('/Details?asset=' + record.asset[0] + '&&net=Ethereum')}>Details</Button>
            </div>)
        }
    ];
    ...
    return (
        <div className='min-h-full'>
            ...
            <div className='box-border  py-[64px] px-[32px] md:px-[11.2rem] rounded-[1rem]'>
                <div className='text-[24px] md:text-[3.2rem] font-bold '>Ethereum Market </div>
                <div className='hidden md:flex mt-[16px] mb-[8px] text-[16px] font-normal text-[#5F6D7E] '>
                    <div className='md:w-[17.4rem] mr-[16px]'>Total market size</div>
                    <div className='md:w-[17.4rem] mr-[16px]'>Total avalible</div>
                    <div className='md:w-[17.4rem]'>Total borrows</div>
                </div>
                <div className='hidden md:flex text-[22px] text-[#272D37] font-semibold'>
                    <div className='md:w-[17.4rem] mr-[1.6rem]'>${marketsize}</div>
                    <div className='md:w-[17.4rem] mr-[1.6rem]'>${available}</div>
                    <div>${tborrows}</div>
                </div>
                ...
                {loading ? <Skeleton className='hidden md:block' loading={loading} active></Skeleton> :
                    <div className='hidden md:block w-[100%] mt-[64px] rounded-[1px] border border-solid border-[#b0b6bd] font-bold ' >
